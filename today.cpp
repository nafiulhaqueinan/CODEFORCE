...
int darage[maxm] , ss , mm , po;  
queue<int> q;  
vector<int> g[maxm] , z[maxm];  
int pedaret[maxm];  
multiset<int> st , wtf , ajab;  
int rp[maxm];  
pi w[maxm],ww[maxm];  
//ll rw[maxm][maxm];  
int dp[maxm];  
int lft[maxm] , rgh[maxm];  
map<int,int> mp,pm,lx,mo,om;  
vector<int> vec;  
