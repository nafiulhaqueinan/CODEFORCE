...
int calCookies(int x, int y, int d) {
    bool isDivisible = ((y - x) % d) == 0;
    int ans=(y - x) / d + 1 - isDivisible;
    return ans;
}
...
void solve(){
     int n, m, d;
        ...
        vint sellers(m);
       fr(i,m){cin>>sellers[i];}
        int inAns = 0;
        if (sellers[0] != 1) {
            inAns = 1;
            sellers.insert(sellers.begin(), 1);
        }
        sellers.pb(n + 1);
n = sellers.size();
int ans = 0;
for (int i = 1; i < n; ++i) {
    ans += calCookies(sellers[i - 1], sellers[i], d);
}
...
for (int i = 1; i < n - 1; ++i) {
    int cur = calCookies(sellers[i - 1], sellers[i + 1], d) - calCookies(sellers[i - 1], sellers[i], d) - calCookies(sellers[i], sellers[i + 1], d);
    newAns = a2 + cur;
    ans = min(ans, newAns);
}
